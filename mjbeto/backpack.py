...
第一种方法为dp[i][j]表示i容量的背包在j件物品中能取的最大值。分两种情况：一种情况为不要第j件物品，则最优情况为背包容量为i时前j－1件物品的最
优值；第二种情况为要第j件物品，则最优情况为背包容量为i-第j件物品体积时前j－1件物品的最优值加上第j件物品的值，这种情况要求i-第j件物品
体积 >= 0。取两种情况里面较大的作为dp[i][j]的值。
第二种方法dp[i][j]表示前i件物品能否填满容量为j的背包。分两种情况：一种情况为不要第i件物品，则dp[i][j]=dp[i-1][j]；第二种情况为要第j件
物品，则dp[i][j] = dp[i-1][j - A[j]],不过这种情况要求j - A[j]>0。取两种情况里面较大的作为dp[i][j]的值。
