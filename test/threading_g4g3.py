...
def thread_task1(lock):
    """
    self, if make the lock non-blocking, then the same as previous example
    task for thread
    calls increment function 100000 times.
    """
    for _ in range(100000):
        have_lock = lock.acquire(False)
        increment()
        if have_lock:
            lock.release()
...
Then, lock is passed as target function argument:
  t1 = threading.Thread(target=thread_task, args=(lock,))
  t2 = threading.Thread(target=thread_task, args=(lock,))
In the critical section of target function, we apply lock using lock.acquire() method. As soon as a lock is acquired, 
no other thread can access the critical section (here, increment function) until the lock is released using 
lock.release() method.
  lock.acquire()
  increment()
  lock.release()
